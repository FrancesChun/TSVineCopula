...
#' @description Generate A D-vine array with 1:d on diagonal
#' @param d Number of variables, must be an integer
#' @return A D-vine array with 1:d on diagonal
dvine <- function(d){
...
#' @description Generate A Bivariate M-vine array with 
#'              X_1, Y_1, ..., X_{k+1}, Y_{k+1} on diagonal
#' @param k Markov order k >= 1, default is k = 1, must be an integer
#' @return A Bivariate M-vine array
...
# Example: Bivariate 
bimvine_matrix_example <- function(){
...
# Example: Trivariate 
trimvine_matrix_example <- function(){
...
#' @description     Helper function, return corresponding index 
...
#' @return          Corresponding index, a vector with the same length as index. 
...
#' @description Testing whether the given upper triangular matrix is a valid 
#'              Markov order 1 S-vine matrix.
#'              Requires function RVineMatrixCheck from package VineCopula.
#' @param M     A 2dx2d vine matrix.
#' @return code  1 for OK;
...
SVineMatrix_mk1_Check <- function(M){
  ...
  if(code == 1){
    ...
    # var_order = diag(M)
    # index = match(mat_l_ele, var_order)
    # index_2 = match(mat_r_ele, var_order)
    # if(any(index_2 != (index + d))){
    #   code = -5
    # }
...
# Example:  
SVineMatrix_mk1_Check_example <- function(){
  d2_mk1 = matrix(c(1,1,1,3,
                    NA,2,2,1,
                    NA,NA,3,2,
                    NA,NA,NA,4), 
...
#' @description Helper function, attempt to fill in all the cells in a given column
#'              requires permn from package 
#' @param mat     A 2dx2d vine matrix.
#' @param k       kth Column, an integer.
#' @return        List of possible matrices. 
generate_specific_upper_tri_matrices <- function(mat, k) {
  ...
  if(length(possible_values)==1){
    ...
    combinations <- do.call(rbind, permn(possible_values))
  }
...
#' @description Helper function, extend column d+2:2d and corresponding rows
#' @param mat     A 2dx2d vine matrix where the upper 1:d+1 columns have been filled.
#' @return        A filled 2dx2d vine matrix. 
...
#' @param mat   A 2dx2d vine matrix where the upper 1:d+1 columns have been filled.
#' @return      A filled 2dx2d vine matrix if it is a valid svine array. NULL 
#'              otherwise. 
...
#' @description Generate all possible d-dimensional Markov order 1 S-vine array 
#'              with 1=X_1, 2=X_2,...,d=X_d, d+1=X_1',...,2d=X_d' on diagonal
#' @param d dimension k >= 2, default is k = 2, must be an integer
#' @return List of 2dx2d S-vine arrays 
