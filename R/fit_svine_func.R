...
#proj = "C:/Users/cheng/OneDrive - UBC/Research"

source(file.path(proj, "simulation", "mvine-array.R"))



#' @description Helper function for fit_svine
...
#' @return an extended (N−k)×d(k+1) matrix with variables in the order specified
...
#' @description Find the best fitted bivariate copulas for Markov order k, d-variate model
#' @param dat matrix with number of columns = d; each column corresponds to a 
#'            univariate vector
#' @param k Markov order k >= 1, default is k = 1
#' @param ... additional arguments pass to RVineCopSelect
#' @return best fitted families, parameter estimates, negative loglikelihood
#' Note: require functions from mvine-array.R
fit_svine = function(dat, k = 1, ...){
  ...
  for(mat in d_matrix_list){
    ...
    for(i in var_order){
      ...
      fitRvine = RVineCopSelect(data = df, Matrix = mk_d_mat, ...)
      fitRvine$var_order = i
      
      # compute Markov order k-1 model
      df_sub = df[,1:(d*k)]
      mat_sub = mk_d_mat[1:(d*k),1:(d*k)]
      fitRvine_sub = RVineCopSelect(data = df_sub, Matrix = mat_sub, ...)
