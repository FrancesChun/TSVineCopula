...
#' @description simulate d-variate time series based on innovations
...
#' @return simulated time series (nxd matrix)
  ...
  for(i in (k+1):n){
    ...
    if(k==1){
      ...
      for(j in 1:d){
        ...
        umat[i,j] = BiCopHinv1(u1 = umat[i-1,j], u2 = innov[i, j], 
...
# Example (sample size at least 300 for copula model fitting) 
innov_mk2_ex = function(){
  ...
  source(file.path(proj, "fit-copula", "fit_innov_func.R"))
